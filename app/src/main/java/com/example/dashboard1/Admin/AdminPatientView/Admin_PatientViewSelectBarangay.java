...
public class Admin_PatientViewSelectBarangay extends AppCompatActivity {
    RecyclerView recyclerView;
    public static String act;
    sbrgyAdapter adapter;
    ...
    boolean result;
    int delay = 2 * 1000;
    final Loadingdialog loadingdialog = new Loadingdialog(Admin_PatientViewSelectBarangay.this);
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin_patient_view_select_barangayh);
        getact();
        builder1 = new AlertDialog.Builder(this);
        initinternetdia();
        result = isOnline();
        recyclerView = findViewById(R.id.rv_pviewbrgy);
        ...
        FirebaseRecyclerOptions<sbrgyModel> options =
                new FirebaseRecyclerOptions.Builder<sbrgyModel>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("barangay"), sbrgyModel.class)
                        .build();
        adapter = new sbrgyAdapter(options);
    ...
    private void txtSearch (String str){
        FirebaseRecyclerOptions<sbrgyModel> options =
                new FirebaseRecyclerOptions.Builder<sbrgyModel>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("barangay").orderByChild("name").startAt(str).endAt(str+ "\uf8ff"), sbrgyModel.class)
                        .build();
        adapter = new sbrgyAdapter(options);
        adapter.startListening();
        recyclerView.setAdapter(adapter);
